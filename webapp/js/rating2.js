document.addEventListener('DOMContentLoaded', function() {
    ...
    let hospitalCommentStatus = {}; // 跟踪每个医院的评论状态
    ...
    function updateHospitalInfo(hospital) {
        ...
        // 根据选择的医院显示相关信息
        const hospitalData = {
            'A': { title: '医院 A', meta: '位置: 城市 X', description: '这是医院 A 的简要介绍...', rating: 4 },
            'B': { title: '医院 B', meta: '位置: 城市 Y', description: '这是医院 B 的简要介绍...', rating: 3 },
            'C': { title: '医院 C', meta: '位置: 城市 Z', description: '这是医院 C 的简要介绍...', rating: 5 },
            'D': { title: '医院 D', meta: '位置: 城市 W', description: '这是医院 D 的简要介绍...', rating: 2 }
        };
    ...
    function updateComments(hospital) {
        ...
        const commentStatus = hospitalCommentStatus[hospital] || { isCommentSubmitted: false, userRating: 0, userCommentText: '' };
        ...
        if (comments[hospital]) {
            ...
            commentsSection.innerHTML = `
                <h3>评论</h3>
                ${commentsHtml}
                <div class="comment-form ${commentStatus.isCommentSubmitted ? 'read-only' : ''}">
                    <h4>添加评论</h4>
                    ...
                    <textarea rows="4" placeholder="在此输入您的评论..." ${commentStatus.isCommentSubmitted ? 'disabled' : ''}></textarea>
                    <button id="submit-comment">${commentStatus.isCommentSubmitted ? '您已提交评论' : '提交评论'}</button>
            ...
            ratingStars.forEach(star => {
                star.addEventListener('click', function() {
                    if (!commentStatus.isCommentSubmitted) {
                        commentStatus.userRating = this.getAttribute('data-value');
                        ratingStars.forEach(s => {
                            s.classList.remove('active');
                            if (parseInt(s.getAttribute('data-value')) <= commentStatus.userRating) {
            ...
            document.querySelector('.comment-form textarea').addEventListener('input', function() {
                commentStatus.userCommentText = this.value;
            });
            ...
            document.getElementById('submit-comment').addEventListener('click', function() {
                if (!commentStatus.isCommentSubmitted) {
                    ...
                    commentFixedRating.innerHTML = `
                        ${[1, 2, 3, 4, 5].map(i => `
                            <label class="${i <= commentStatus.userRating ? 'active' : ''}">&#9733;</label>
                        `).join('')}
                    `;
                    commentsSection.innerHTML += `
                        <div class="comment">
                            <p class="comment-meta">您 - ${formattedDate}</p>
                            <p>${commentStatus.userCommentText}</p>
                        </div>
                    `;
                    ...
                    // 更新评论状态
                    commentStatus.isCommentSubmitted = true;
                    hospitalCommentStatus[hospital] = commentStatus; // 保存状态
                } else {
                    this.textContent = '您已提交评论';
                }
            });
